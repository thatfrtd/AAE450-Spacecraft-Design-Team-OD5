...
% Description: Simulate deorbit orbits with J2 perturbations, thrust, and 
% drag with sampled drag coefficients in order to analyze distribution of
% landing sites for safety analysis.
...
a_d = @(t, x, C_D) a_d_J2(t,x) + cartesian_to_RTN_DCM_from_cart(x, mu_E) * a_d_thrust(t,x) ...
...
parfor i = 1 : N
    C_D_i = unifrnd(2, 2.8);
    [t_keplerian_i, x_keplerian_cartesian_i] = ode45(@(t,x) gauss_planetary_eqn(f0_cartesian(x, mu_E), B_cartesian(x, mu_E), a_d(t, x, C_D_i)), tspan, x0_cartesian, tolerances);
    x_keplerian_cartesian_i = x_keplerian_cartesian_i';
end
%%
