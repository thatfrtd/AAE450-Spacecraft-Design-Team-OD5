...
spacecraft_params.thrust_directions = {{}}; % RCS thrusters
spacecraft_params.tau_max = 15;%spacecraft_params.F_max * spacecraft_params.r * 4; % [N m]
...
nu = 3; % Number of controls (tau_rw)
...
%% Get Dynamics
f = @(t, x, u, p) rotational_3DoF_EoM(t, x, u, p, [spacecraft_params.I]);

%% Specify Constraints
max_ang_vel = deg2rad(20);
...
max_reaction_wheel_torque_constraint = {1:N, @(t, x, u, p) norm(u(1:3)) - tau_max_nd};
control_convex_constraints = {max_reaction_wheel_torque_constraint};
...
%% Specify Objective
objective_min_fuel = @(x, u, p, x_ref, u_ref, p_ref) sum(norms(u(1:3, :))) * delta_t;
