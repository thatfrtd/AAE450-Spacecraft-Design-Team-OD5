...
for k = 1 : N
    [~, min_safety(k), x_safety_ck(:, :, k)] = construct_passive_safety_constraint(ptr_sol.x(:, k, i), ptr_sol.x(:, k, i), @(t_prop, x_prop) f_opt(t_prop + t_k(k), x_prop, zeros([nu, 1]), []), @(t_prop, x_prop) A_func(t_prop + t_k(k), x_prop, zeros([nu, 1]), []), @(t_prop, x_prop) keep_out_sphere_constraint(t_prop + t_k(k), x_prop, zeros([nu, 1]), []), @(t_prop, x_prop, x_ref_safe) keep_out_sphere_constraint_linearized_func(t_prop + t_k(k), x_prop, zeros([nu, 1]), [], x_ref_safe, zeros([nu, 1]), [], 1), T, N_safe_ck, 1e-3);
end
...
plot_cartesian_orbit(x_cont_sol(1:3,:)', 'k', 0.4, 1, 1); hold on
...
legend("Target", "Drift", 'Spacecraft', "", "RCS Thrust", "Start", "End", "Guess", "Keepout Sphere", 'Location', 'northwest'); axis equal; grid on
...
for i = 1:ptr_sol.converged_i
    ...
    linestyle(i) = "-";
    plot_cartesian_orbit(x_iters{i}(1:3,:)', rgb2hex(C(i, :)), 0.4, 1, 1); hold on
end
