function [passive_safety_constraint, min_safety, x_prop] = construct_passive_safety_constraint_stricter(x, x_0, f, A, safety_constraint, safety_constraint_linearized, T, N_safe, integration_tolerance)
...
unsafe_i = find(safety > 0);

% Calculate STM from initial to least safe time
A_min_safety = integrate_STM(x_0, f, A, [0, z_min_safety], tolerances);

% Calculate STM from initial to unsafe times
A_unsafe = zeros([numel(x_0), numel(x_0), numel(unsafe_i)]);
for un = 1 : numel(unsafe_i)
    A_unsafe(:, :, un) = integrate_STM(x_0, f, A, [0, z_k(unsafe_i(un))], tolerances);
end

% Define (approximate) safety constraint using STM to map perturbed initial
% to perturbed state at min safety
% Approximate because only doing the worst timesteps
if ~isempty(unsafe_i)
    for un = 1 : numel(unsafe_i)
        passive_safety_constraint(un) = safety_constraint_linearized(z_k(unsafe_i(un)), squeeze(A_unsafe(:, :, un)) * x, x_prop(:, unsafe_i(un)));
    end
else
