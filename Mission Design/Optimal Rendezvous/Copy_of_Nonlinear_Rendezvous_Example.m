...
%% Get Dynamics
f_nonlinear = @(t, x, u, p) nonlinear_relative_orbit_EoM(t * char_star.t, x, u, p, [x_keplerian_c; spacecraft_params.Isp]);
...
%% Boundary conditions
initial_bc = @(x, p) [x - x_0 ./ nd_scalar];
terminal_bc = @(x, p, x_ref, p_ref) [x(1:6) - x_f ./ nd_scalar(1:6); 0]; % Don't constrain final mass

%% Specify Objective
objective_min_fuel = @(x, u, p, x_ref, u_ref, p_ref) sum(norms(u)) * delta_t / char_star.t * char_star.F / (spacecraft_params.Isp * 9.81e-3) * 1000;

%% Create Guess
% Straight Line Initial Guess
guess.x = (linspace(0, 1, N) .* (x_0 - [x_f; x_0(7)]) + x_0) ./ nd_scalar;
guess.u = ones([3, Nu]) * 1e-6;
guess.p = [];

%% Construct Problem Object
problem = DeterministicProblem(x_0 ./ nd_scalar, x_f ./ nd_scalar(1:6), N, u_hold, tf, f_opt, guess, convex_constraints, objective_min_fuel, scale = scale, nonconvex_constraints = nonconvex_constraints, initial_bc = initial_bc, terminal_bc = terminal_bc, integration_tolerance = 1e-12, discretization_method = "error", N_sub = 1, Name = "nonlinear_rendezvous");

% [problem, Delta_disc] = problem.discretize(guess.x, guess.u, guess.p);

%%
[problem, Delta_disc] = problem.discretize(ptr_sol_c.x(:, :, ind_c), ptr_sol_c.u(:, :, ind_c), []);
...
scatter3(r_f(1), r_f(2), r_f(3), 48, "red", "x"); hold off
