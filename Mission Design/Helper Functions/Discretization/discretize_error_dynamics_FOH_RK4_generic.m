function [A_k, B_k_plus, B_k_minus, S_k, d_k, Delta] = discretize_error_dynamics_FOH_RK4_generic(f, A, B, S, N, tspan, x_ref, u_ref, s_ref, N_sub)
    % Discretization of the devition from a reference trajectory for a
    % dynamical system assuming FOH control.
    ...
    a{3} = [0, 1 / 2];
    a{4} = [0, 0, 1];
    b = [1 / 6, 1 / 3, 1 / 3, 1 / 6]; % High order, Propagation stage
