function [transfer] = QLaw_transfer(x_keplerian_d, x_keplerian_c, mu, spacecraft_params, Q_params, Qdot_opt_params, options)
...
arguments
    ...
    spacecraft_params % S/c Isp, max thrust, initial mass, fuel mass
    Q_params % W_oe, eta_a, eta_r, m, n, r, Theta_rot (reference direction)
    Qdot_opt_params % Parameters for the optimization needed to determine efficiencies
    options.integration_tolerance = 1e-6
    options.angular_step = deg2rad(10) % Advised to be in range [5, 20] deg
    options.R_c = 1 % Quantity controlling penalization of remaining distance from target
    options.iter_max = 1e5 % Maximum number of timesteps
    options.a_disturbance = @(t,x) [0;0;0] % Disturbing accelerations
end
...
F_max_nd = spacecraft_params.F_max / char_star.F;
...
% Prepare integration
tolerances = odeset(RelTol=1e-5, AbsTol=1e-5, Stats = "off");
...
iter = 1;
Q = Q_stop;
while Q(iter) >= Q_stop && iter < options.iter_max
    ...
    % Calculate control
    [D, Qdot, partial_Q_partial_oe] = D_Qdot_partial_Q_partial_oe_func(oe, L, oe_t, W_oe, m, n, r, F_max_nd, penalty_params.W_p, r_p_min_nd, penalty_params.k);
    [u_nd(:, iter), alpha(iter), beta(iter)] = QLaw_thrust_mapping(D, F_max_nd);

    % Determine if thrusting should happen based on heuristic
    [Qdot_min, Qdot_max] = Qdot_extremize(oe, L, partial_Q_partial_oe, Qdot_opt_params);
    ...
    [t_step, x_step] = ode45(@(t, x) [gauss_planetary_eqn(f0_modified_equinoctial(x, 1), B_modified_equinoctial(x, 1), a_control(x) + a_d(t, x .* nd_scalar, mu)); mdot], t_nd(end) + [0, dt_step], x_me_mass_nd(:, end), tolerances);
...
transfer.delta_m = spacecraft_params.m_0 - x_keplerian_mass(7, end);
