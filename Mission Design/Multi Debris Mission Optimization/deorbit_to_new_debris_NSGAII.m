...
Omega_c = deg2rad(320.5520 - 40 ); % [rad] right ascension of ascending node
omega_c = deg2rad(301.2069 - 100 ); % [rad] argument of periapsis
...
ToF1_bounds = [0.1, 2]; % [hr] initial -> intermediate transfer time of flight
ToF2_bounds = [0.1, 2]; % [hr] intermediate to target transfer time of flight
...
% Set up MultiObj
MultiObj.fun = @(x) batch_Lambert_J2_drift_transfer(x0_d_keplerian, [x(:, 3)' * char_star.l; x(:, 4:5)'; repmat(x0_d_keplerian(4:6), 1, size(x, 1))], x0_c_keplerian, x(:, 1:2)' * 3600, 20, mu_E, R_E, J_2_val, char_star, 2, 0.3);
...
MultiObj.title = "Lambert Delta V vs ToF Pareto Front Optimization Using NSGA-II";
MultiObj.plot_bounds = [0, 0.3;
                        0, 2];
...
params.ms = 0.01;       % Mutation strength
