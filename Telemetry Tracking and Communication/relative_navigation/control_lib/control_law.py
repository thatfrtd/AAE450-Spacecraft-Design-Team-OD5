...
Eq 53:  τ_cmd   = K_θ · θ_C_des + K_ω · (ω_des − ω_C)
Eq 54:  θ_C_des = q_des − q^{C→I}   (vector part of quat difference)
"""
import numpy as np
...
def control_law(x_est, w_C, r_des, v_des, q_des, w_des, I_c):
    ...
    w_C     : (3,)  true chaser angular velocity from gyro [rad/s]
    ...
    Returns
    -------
    u_cmd   : (3,)  translational Δv command [km/s², BODY frame]  ← Eq 52
    tau_cmd : (3,)  torque command [kg·km²/s²]                    ← Eq 53
    """
    r_C = x_est[13:16]
    v_C = x_est[16:19]
    q_C = x_est[19:23]   # q^{I→C}

    # ── Translational gains (km units) ───────────────────────────────────────
    # ω_n = 0.01 rad/s, ζ = 1  →  period ≈ 628 s
    omega_n = 0.05    # K_r = 2.5e-3, K_v = 0.10
    zeta    = 1.0
    K_r = (omega_n**2)         * np.eye(3)   # 1e-4  [1/s²]
    K_v = (2 * zeta * omega_n) * np.eye(3)   # 0.02  [1/s]

    # ── Attitude gains ────────────────────────────────────────────────────────
    omega_n_att = 0.05
    zeta_att    = 1.0
    K_theta = (omega_n_att**2)              * np.eye(3)
    K_w     = (2 * zeta_att * omega_n_att)  * np.eye(3)

    # ── Eq 51: required Δv in INERTIAL frame ─────────────────────────────────
    dv_rqd = K_r @ (r_des - r_C) + K_v @ (v_des - v_C)

    # ── Eq 52: rotate to CHASER BODY frame ───────────────────────────────────
    T_IC  = quat2dcm(q_C)       # inertial → chaser body
    u_cmd = T_IC @ dv_rqd       # [km/s²], body frame

    # ── Eq 54: attitude error = vector part of (q_des − q^{C→I}) ─────────────
