...
%    (b) Earth-facing X-face  (A_Xface    = 4 m^2): Earth IR only (nadir)
...
% (a) Sun-facing Y-face: solar + albedo + Earth IR
...
% (b) Earth-facing X-face: Earth IR only
Q_ext_hot_earth = eps_MLI * hot.q_IR * hot.F_earth * A_Xface;

func_hot_earth = @(T) Q_ext_hot_earth ...
    - eps_MLI * sigma * A_Xface * (T^4 - T_space^4) ...
    - A_Xface * (T - T_int) / R_spec_hot;
...
A_shadow_hot = A_total - A_hot_face - A_Xface;   % 32 - 6 - 4 = 22 m^2
...
Q_leak_in = A_hot_face   * (T_sun_K        - T_int) / R_spec_hot ...
          + A_Xface       * (T_earth_hot_K  - T_int) / R_spec_hot ...
